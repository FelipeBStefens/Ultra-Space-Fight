// Imports the function for making the user login API call;
import { getUserLogin } from "../Utils/scriptFetch.js";
// Imports the client-side validation functions for email and password;
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // Attaches an event listener to the button click for form submission;
    button.addEventListener("click", async (event) => {
        ...
        if (!validateInputs(emailInput, passwordInput, button)) {
            // Alerts and stops the submission if inputs are invalid;
        ...
        // Finds the container and error elements to pass to the login API function for server-side error handling;
        const emailContainer = emailInput.closest(".inputContainer");
        const passwordContainer = passwordInput.closest(".inputContainer");
        const emailError = emailContainer.querySelector(".errorMessage");
        const passwordError = passwordContainer.querySelector(".errorMessage");

        // Calls the asynchronous login function, passing inputs and UI elements;
        ...
        // Checks if the login was successful (user object returned);
        if (user != null) {
